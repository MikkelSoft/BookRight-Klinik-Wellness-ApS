    ...
    public class KampagneRabat : IRabatBeregner
    {
        public RabatResultat? BeregnRabat(RabatKontekst kontekst)
        {
            ...
            var bedsteKampagne = kontekst.AktiveKampagner
                .Where(k => k.ErAktiv(kontekst.BookingDato) && k.GælderForBehandling(kontekst.BehandlingsType))
                .OrderByDescending(k => k.RabatProcent)
                .FirstOrDefault();

            if (bedsteKampagne is null)
                return null;

            decimal pris = kontekst.BasisPris * (1 - bedsteKampagne.RabatProcent);
            return new RabatResultat($"Kampagne: {bedsteKampagne.Navn} ({bedsteKampagne.RabatProcent * 100:0}%)", bedsteKampagne.RabatProcent, pris);
        }
    }
}
